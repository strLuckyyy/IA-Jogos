...
func _init() -> void:
	# pesos e viés começam aleatórios
...
func predict(entry: Array[float]) -> float:
	...
	# sigmoide pra jogar tudo entre 0 e 1
	return 1.0 / (1.0 + exp(-sum))


func mutate(mutation_rate: float) -> void:
	# mexe um pouco nos pesos pra explorar coisas novas
...
func cross_data(other_brain: NeuralNetwork) -> NeuralNetwork:
	...
	# cada peso vem de um dos dois pais, 50/50
	for i in range(INPUT_COUNT):
